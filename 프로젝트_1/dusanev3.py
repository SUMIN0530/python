...
region_name = '예천태양광_1'  # 분석할 지역
region_path = os.path.join(base_path, region_name)
excel_file = os.path.join(region_path, "예측결과_및_모델평가.xlsx")

# -----------------------------
# 데이터 불러오기
# -----------------------------
df_pred = pd.read_excel(excel_file, sheet_name='예측결과')
df_eval = pd.read_excel(excel_file, sheet_name='모델평가')

# -----------------------------
# Ensemble 기반 SMAPE_i 계산 함수
# -----------------------------
...
def smape(a, f):
    ...
    mask = denom != 0
    return np.abs(a[mask] - f[mask]) / denom[mask] * 100  # 개별 SMAPE
...
df_weather_list = [pd.read_excel(os.path.join(region_path, f))
...
df_merged = pd.merge(
    df_pred,
    df_weather[['일시', 'solar']],  # solar 컬럼 사용
    on='일시',
    how='left'
)
...
plt.title(f"{region_name} Ensemble 기반 상관 히트맵", fontsize=13)
plt.tight_layout()

# -----------------------------
# 이미지 저장
# -----------------------------
heatmap_path = os.path.join(region_path, "Ensemble_heatmap_corr.png")
plt.savefig(heatmap_path, dpi=300)
plt.close()

print(f"[완료] {region_name} 히트맵 저장 → {heatmap_path}")
