...
def draw_centered_text(img, text, font, scale, color, thicknes=1):
    ...
    # text_height : 텍스트가 차지하는 세로(글자 위 ~ baseline) 픽셀

    # baseline : 글자의 아래쪽(g, y, p 같은 내려오는 꼬리)을 위한 추가 여백 픽셀
...
draw_centered_text(canvas, "Centerd Text", cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255,255,255))
...
while True:
    ...
    if key == ord('q'):
        ...
        print(f'키 코드 {key}, 문자 : {chr(key)}') # 더 있었음
